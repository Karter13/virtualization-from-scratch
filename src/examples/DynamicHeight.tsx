...
function validateProps(props: UseDynamicSizeListProps) {
    ...
    if (!itemHeight && !estimateItemHeight) {
        throw new Error(
            `you must pass enter "itemHeight" or "estimateItemHeight" prop`
        )
    }
}
...
export function useDynamicSizeList(props: UseDynamicSizeListProps) {
    ...
    }, [scrollTop, overScan, listHeight, itemsCount, itemHeight, estimateItemHeight, measurementCache, getItemKey]);

    const latestData = useLatest({measurementCache, getItemKey, allItems, getScrollElement, scrollTop});
